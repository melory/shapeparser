/*
 * 
 * @Description: 对shapefile数据中feature的相关操作
 * 
 */
...
public class FeatureUtils {
	...
	private static final Log log = LogFactory.getLog(FeatureUtils.class);
	...
	public static String[][] getFeatures(DataStore dataStore,
		...
		if (startId < 1) {
			log.info("输入的startid = " + startId + "小于1，已经自动置为1。");
			startId = 1;
		}
		...
		if (featureNum < startId) { // feature的总数目比startid还小的情况
			log.info("满足条件的feature总数目少于" + startId + "条，结果将只会返回data[][]的前两行。");
			...
			if (featureNum >= featureSize + startId - 1) {
				...
				log.info("featureSize在startId的基础上过大，结果将返回从startId记录开始之后的所有记录。");
